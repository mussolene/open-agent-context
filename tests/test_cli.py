...
def test_cli_repo_auto_memory_flow(tmp_path):
    db = tmp_path / "oacs.db"
    runner = CliRunner()
    assert runner.invoke(app, ["init", "--db", str(db), "--json"]).exit_code == 0
    assert (
    ...
    assert start_payload["auto_memory_policy"]["start_writes_memory"] is False
    assert "capsule" in start_payload
    ...
    finish = runner.invoke(
        ...
        [
            ...
            "add auto memory",
            "--summary",
            "repo auto-finish stores a D1 episode",
            "--outcome",
            "implemented",
            "--json",
        ],
    ...
    assert finish_payload["memory_id"].startswith("mem_")
    assert finish_payload["auto_memory_policy"]["committed_depth"] == 1
    assert finish_payload["auto_memory_policy"]["d2_d3_auto_commit"] is False
    ...
    assert finish_payload["memory_id"] in query.output


def test_cli_repo_autorun_captures_command_result(tmp_path):
    db = tmp_path / "oacs.db"
    runner = CliRunner()
    assert runner.invoke(app, ["init", "--db", str(db), "--json"]).exit_code == 0
    assert (
    ...
    assert "42" in payload["stdout_tail"]
