...
Superseded note: this report predates the OACS v0.2.0 `memory_calls` refactor.
Use `memory_calls_gemma_e2b_2026-05-01.md` for current benchmark conclusions.

This report compares current-task-only prompting, raw full-context prompting,
the current OACS Context Capsule path, and the MCP-like OACS memory-call path.
...
| Synthetic memory-critical | Small controlled memory | `oacs_memory_call_loop` | 0/5 | 1.0 | 800 | 1,953 | 2,753 | 43.7s |
...
| MemoryArena group travel | Medium structured reuse | `oacs_memory_call_loop` | 5/5 | 5.0 | 2,296 | 2,743 | 5,039 | 60.3s |
...
| MemoryArena progressive search | Large accumulated context | `oacs_memory_call_loop` | 5/5 | 5.0 | 4,959 | 5,755 | 10,714 | 66.5s |
...
| AMA-Bench open-ended QA | Long trajectory reasoning | `oacs_memory_call_loop` | 5/5 | 5.0 | 1,754 | 3,238 | 4,992 | 59.0s |

### What Improved

- Medium and large public memory tasks benefit from deterministic memory_calls.
  Compared with raw full-context prompting, `oacs_memory_call_loop` kept 5/5
...
- Long-context wall time improved where the memory-call layer reduced prompt size:
...
- This run predates OACS v0.2.0 structured `memory_calls`; current results are
  recorded in the v0.2.0 memory calls report.
...
The main architecture gap at the time of this run was benchmark-specific parsing
inside `oacs/loop/memory_calls.py`. The later OACS v0.2.0 implementation removed
that parsing from the core loop and made structured `memory_calls` the benchmark
contract.
...
- Medium context: OACS memory_calls start paying off when reusable evidence can
  replace several kilotokens of raw trace.
- Large accumulated context: OACS memory_calls are materially better. Progressive
...
Superseded note: этот отчёт был сделан до refactor OACS v0.2.0 `memory_calls`.
Актуальные выводы см. в `memory_calls_gemma_e2b_2026-05-01.md`.

Этот отчёт сравнивает current-task-only prompting, raw full-context prompting,
текущий OACS Context Capsule path и MCP-like OACS memory-call path. Это отчёт
по reference implementation, а не изменение стандарта OACS.

### Что стало лучше

- На medium и large public memory tasks deterministic memory_calls дают пользу.
  По сравнению с raw full context `oacs_memory_call_loop` сохранил 5/5 success и
...
- На long-context задачах wall time тоже снизился там, где memory-call layer уменьшил
...
- Старый memory-call extractor провалил synthetic suite в этом прогоне. Этот gap
  закрыт последующим structured `memory_calls` path в OACS v0.2.0.
...
Главный архитектурный gap на момент этого прогона был в benchmark-specific
parsing внутри `oacs/loop/memory_calls.py`. Последующая реализация OACS v0.2.0
убрала этот parsing из core loop и сделала structured `memory_calls` benchmark
contract.
...
- Medium context: OACS memory_calls начинают окупаться, когда reusable evidence
  заменяет несколько kilotokens raw trace.
- Large accumulated context: OACS memory_calls дают существенную пользу.
