...
Reference implementation v0.2.2 includes `memory_calls` as a backend-independent
operation trace for memory work in the general MemoryLoopEngine, CLI, API, and
benchmark runner. Native model `tool_calls` may be used by a future adapter, but
OACS does not require backend tool-calling support to query, read, extract
evidence, audit, or build capsules.
...
Reference implementation v0.2.2 включает `memory_calls` как backend-independent
operation trace для работы с памятью в общем MemoryLoopEngine, CLI, API и
benchmark runner. Native model `tool_calls` может использоваться будущим
