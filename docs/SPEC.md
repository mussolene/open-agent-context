...
EvidenceRef, AuditEvent, SecretRecord, SensitiveFact, ProtectedRef, TaskTrace,
ExperienceTrace, MemoryLoopRun.
...
Reference implementation v0.3.5 includes `memory_calls` as a backend-independent
...
v0.3.5 includes operation envelopes for `MemoryOperation`, `ContextOperation`,
...
Protected values are not ordinary memory. `SecretRecord` and `SensitiveFact`
hold encrypted values; `ProtectedRef` is the portable reference projected into
context, tool bindings, evidence, or audit metadata. `protected.use` allows
adapter use without revealing plaintext, while `protected.read` and
`secret.read` explicitly reveal plaintext and should be narrowly granted.
Context Capsules, ToolCallResult, EvidenceRef public payloads, and AuditEvent
metadata must not persist plaintext protected values.
...
Reference implementation v0.3.5 включает `memory_calls` как backend-independent
...
v0.3.5 включает operation envelopes для `MemoryOperation`, `ContextOperation`,
...
Protected values не являются обычной memory. `SecretRecord` и `SensitiveFact`
хранят encrypted values; `ProtectedRef` является portable reference, которая
проецируется в context, tool bindings, evidence или audit metadata.
`protected.use` разрешает adapter use без раскрытия plaintext, а
`protected.read` и `secret.read` явно раскрывают plaintext и должны выдаваться
узко. Context Capsules, ToolCallResult, public payloads EvidenceRef и metadata
AuditEvent не должны сохранять plaintext protected values.
