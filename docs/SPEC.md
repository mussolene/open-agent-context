...
Reference implementation v0.2.8 includes `memory_calls` as a backend-independent
...
v0.2.8 defines operation envelopes for `MemoryOperation`, `ContextOperation`,
...
Reference implementation v0.2.8 включает `memory_calls` как backend-independent
...
v0.2.8 задаёт operation envelopes для `MemoryOperation`, `ContextOperation`,
