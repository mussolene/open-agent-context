...
Reference implementation v0.2.6 includes `memory_calls` as a backend-independent
...
v0.2.6 defines operation envelopes for `MemoryOperation`, `ContextOperation`,
...
Reference implementation v0.2.6 включает `memory_calls` как backend-independent
...
v0.2.6 задаёт operation envelopes для `MemoryOperation`, `ContextOperation`,
